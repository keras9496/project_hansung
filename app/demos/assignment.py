...
우측: 배정 결과 테이블 + 재배정 필요 목록 (AI 협상안 생성 포함).

내부 상태값(Application.status, MailLog.event_kind 등)은 'deadlock' 으로
유지하지만, 사용자 노출 문구는 모두 '재배정 필요' 로 통일한다.
"""
...
def _left_pane(semester: Semester) -> None:
    ...
    st.markdown(
        ...
        "전공 건물에 가능 후보가 없으면 다른 건물 fallback 없이 **재배정 필요**  \n"
    ...
    if last:
        ...
        st.success(
            ...
            f"**{last['deadlock']}건 재배정 필요**{match_line}"
        )
        ...
        with st.expander("실행 상세 로그", expanded=False):
            for d in last["details"]:
                if d["result"] == "assigned":
                    ...
                    st.write(
                        f"- #{d['application_id']} {d['applicant']} → **재배정 필요**"
                        + (f" — {reason}" if reason else "")
                    )


def _right_pane(semester: Semester) -> None:
    st.subheader("📋 배정 결과 / 재배정 필요")
    ...
    if deadlocks:
        st.markdown("---")
        st.error(
            f"⚠️ 재배정 필요 {len(deadlocks)}건 — 신청 조건(요일/시간/종류/수용인원)을 "
        ...
        st.caption(
            "각 재배정 필요 건을 펼쳐 'AI 협상안 생성' 을 누르면 Claude 가 가용 슬롯을 분석해 "
